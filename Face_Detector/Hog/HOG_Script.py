...
class Hog:
    ...
    def extract_histogram_of_oriented_gradients(im):
        ...
        x_filter, y_filter = Hog.sobel_filter()
        dx = Hog.image_filtering(im, x_filter)
        dy = Hog.image_filtering(im, y_filter)
        mag_matrix, angle_mat = Hog.magnitude_direction_gradient(dx, dy)
        histogramMat = Hog.create_histogram(mag_matrix, angle_mat, 8)
        hog_descriptors = Hog.block_normalization(histogramMat, 2)
    ...
    def facedetection(I_target, I_template):
        template_HOG = Hog.extract_histogram_of_oriented_gradients(I_template)
        ...
        while x + box_h <= img_h:
            ...
            while y + box_w <= img_w:
                ...
                img_HOG = Hog.extract_histogram_of_oriented_gradients(img_window)
        ...
        while thresholded_boxes != []:
            ...
            for box in thresholded_boxes:
                if Hog.iou(currBox, box, box_w) >= 0.5:
                    toRemove.append(box)
    ...
    @staticmethod
    def detect_hog(I_target_gray_scale,I_target_color,I_template_gray_scale):
        bounding_boxes=Hog.facedetection(I_target_gray_scale, I_template_gray_scale)
        return Hog.visualize_face_detection(I_target_color,bounding_boxes,I_template_gray_scale.shape[0])



    # I_target= cv2.imread(r'Face_Detector/Hog/First.png', 0)   #   $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    # I_template = cv2.imread(r'Face_Detector/Hog/sampleinput.png', 0)  #    $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    # bounding_boxes=facedetection(I_target, I_template)

    # I_target_c= cv2.imread(r'Face_Detector/Hog/First.png')       #  $$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
    # bounding_boxes= visualize_face_detection(I_target_c, bounding_boxes, I_template.shape[0])
